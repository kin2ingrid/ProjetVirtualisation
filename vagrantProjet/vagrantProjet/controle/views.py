...
def consolidation(request):
    ...
    for operation in operations:
        ...
        for bordereauop in bordereauops:
            if (
                operation.date == bordereauop.TRANSFER_DATE  and
                operation.montant == bordereauop.TRANSFER_AMOUNT and
                operation.refOp == bordereauop.TRANSFER_ID[-6:] and
    ...
    if request.method == 'POST':
        ...
        if f_operateur.name.endswith('.xlsx'):
            ...
            for row in ws.iter_rows(min_row=10, values_only=True):
                ...
                objet.TRANSFER_DATE = row[0]
                objet.TRANSFER_ID = str(row[1])
                objet.MSISDN  = row[2]
                objet.REFERENCE_NUMBER = row[3]
                objet.STATEMENT = row[4]
                objet.TRANSFER_AMOUNT = row[5]
                objet.CREDIT = row[6]
                objet.DEBIT = row[7]
                objet.FEES = row[8]
                objet.PREVIOUS_BALANCE = row[9]
                objet.POST_BALANCE = row[10]
                objet.save()
